...
Holdout last observations for holdout-patients. Those are predicted in cross-validation.

```{r 10-fold_holdout, echo=FALSE, eval=TRUE, message=FALSE}
library(igraph)
...
{
  # Patients with 'holdout_suffix' as last characted in SUBJECT_ID are held out with their observation from week 12
  holdout_index <- as.vector(as.numeric(endsWith(as.character(sysdimet$SUBJECT_ID), holdout_suffix) & sysdimet$WEEK == 12))
  cvdir <- paste0("/media/jari/Jarin WD Passport/cvmodels/week12/holdout_", holdout_suffix)
...
localfit_directory <- "/media/jari/Jarin WD Passport/cvmodels/holdout_0"
targetname <- "fshdl"

target_blmm <- mebn.get_localfit(targetname, localfit_directory)

# Predictions for week 12 of holdout patients
ms <- rstan::summary(target_blmm, pars=c("Y_pred"), probs=c(0.10, 0.90), na.rm = TRUE)

pred_week12 <- as.vector(ms$summary[,1])
...
true_response <- sysdimet %>%
  filter(SUBJECT_ID %in% holdout_subjects) %>%
  filter(WEEK == 12) %>%
  select("fshdl")

nrmse_rows <- mebn.NRMSE(pred_week12, true_response$fshdl, mean(true_response$fshdl))
nrmse_rows
nrmse <- mean(nrmse_rows)
nrmse
...
true_week12 <- sysdimet %>%
  filter(SUBJECT_ID %in% holdout_subjects) %>%
  filter(WEEK == 12) %>%
  select("fshdl")

sign(true_week12$fshdl - true_week8$fshdl)

sign(pred_week12 - true_week8$fshdl)
