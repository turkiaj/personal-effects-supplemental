...
```{r normal_model_ppc, echo=FALSE, eval=TRUE, message=FALSE, cache=FALSE}
...
sysdimet_normal_ar1 <- mebn.bipartite_model(reaction_graph = initial_graph, 
                                   inputdata = sysdimet,
                                   predictor_columns = assumedpredictors, 
...
and also Normal AR(1) model with RHS prior on betas
...
sysdimet_normal_ar1_rhs <- mebn.bipartite_model(reaction_graph = initial_graph, 
                                   inputdata = sysdimet,
                                   targetdata = holdout_index,
                                   predictor_columns = assumedpredictors, 
                                   assumed_targets = only_fsins, 
...
```{r normal_ar1_rhs_ppc, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}
mebn.target_dens_overlays("BLMM_normal/ar1_rhs/no_holdout/", only_fsins, sysdimet)
```
...
expfam_ar1_dirs[target_models_dirs$Name=="fsins",]$modelcache <- "models/BLMM_normal/ar1"

expfam_ar1_rhs_dirs <- assumedtargets
expfam_ar1_rhs_dirs$modelcache <- "models/BLMM_gamma/ar1_rhs/no_holdout"
expfam_ar1_rhs_dirs[target_models_dirs$Name=="fsins",]$modelcache <- "models/BLMM_normal/ar1_rhs/no_holdout"
```
...
##Comparison with other methods with a custom metrics
...
**MEBN out-of-sample predictions**

```{r k-fold_eval, message=FALSE, warning=FALSE}
source("model_eval_functions.r")
...
```{r model_summary}
summary.mat <- summary.mat[order(rownames(summary.mat)),]
summary.mat[,2:6] <- paste0(summary.mat[,2:6], "%")
