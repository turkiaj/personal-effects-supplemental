...
```{r mse_plot, fig.height = 3, fig.width = 7, echo=FALSE, fig.align = "left", message=FALSE, warning=FALSE, cache=TRUE}
library(ggplot2)
...
label.df <- as.data.frame(paste0(colnames(mse.mat)[2:7], " ", sprintf(as.numeric(mse.mat[6,2:7]), fmt = '%#.2f')))
...
mseplot <- ggplot(mse.df, show.legend = FALSE) +
  ...
  geom_text(data = label.df, aes(label = label, x = "EXPFAM AR(1) RHS", y = value, hjust = -.03, vjust = (label == "fpgluk 0.05")*1.2), size = 3) +
...
```{r accuracy_plot, echo=FALSE, fig.align="left", fig.height=3, fig.width=7, message=FALSE}
library(ggplot2)
library(gridExtra)
library(grid)
...
ggplot() +
  ...
  labs(x = "Blood test", y = "Prediction accuracy") + theme(axis.title.x = element_blank()) +
