---
title: "Revealing the Personal Effects of Nutrition with Mixed-Effect Bayesian Network Supplementary Materials"
author:
...
```{r spagettiplot_fshdl, echo=FALSE, fig.align="left", fig.height=1.5, fig.width=5, message=FALSE}
library(ggplot2)
library(gridExtra)
library(grid)
...
Diffent model versions are compared with a normalized root mean squared error (NRMSE) and also with a classification test that shows better the usefulness of the method. The classification test allows also comparison with other classification methods.
...
#kable(nrmse.mat, "latex", booktabs = T, caption = "") %>%
#  kable_styling(latex_options = c("striped", "condensed"), full_width = TRUE) %>%
#  row_spec(0,bold=TRUE)
...
nrmse.df$model <- factor(nrmse.mat[,1], levels = rev(c("Normal","Gamma","Gamma AR(1)","Gamma AR(1) RHS","EXPFAM AR(1)","EXPFAM AR(1) RHS")))
...
label.df <- cbind(label.df, as.numeric(nrmse.mat[6,2:7]))
...
ggplot(nrmse.df, show.legend = FALSE) +
  ...
  geom_text(data = label.df, aes(label = label, x = "EXPFAM AR(1) RHS", y = value, hjust = .2, vjust = (label == "fpgluk")*1.2+1.2), size = 3) +
  scale_y_continuous() +
  labs(x = "", y = "NRMSE") + 
  coord_flip() +
  theme_bw() +
...
```{r safa_differences, echo=FALSE, eval=TRUE, message=FALSE}
p <- nrow(assumedpredictors)
ins_idx <- match("fsins", assumedtargets$Name)
safa_idx <- match("safa", assumedpredictors$Name)

safa_fsins_idx <- p*(ins_idx-1) + safa_idx

# safa -> fsins for every patient
safa_fsins <- as.data.frame(cbind(personal_effects[, safa_fsins_idx], seq(1, nrow(personal_effects))))
colnames(safa_fsins) <- c("effect", "subject_number")

ordered_safa_fsins <- safa_fsins[order(-safa_fsins$effect),]

person_id1 <- head(ordered_safa_fsins$subject_number,1)
person_id2 <- tail(ordered_safa_fsins$subject_number,1)

#What reaction clusters these patients belong?
#sysdimet[person_id1,]$effect_group   # patient 1 -> cluster 3
#sysdimet[person_id2,]$effect_group   # patient 86 -> cluster 2
...
write.graph(personal_graph1, "graphs/personal_insample_graph_1.graphml", "graphml")
```
...
write.graph(personal_graph2, "graphs/personal_insample_graph_86.graphml", "graphml")
```
...
```{r personal_graphvisu1, eval=TRUE, fig.height = 4, fig.width = 5, fig.align = "left", echo=FALSE, message=FALSE, warning=FALSE}
source("mebn/MEBN.r")
require(igraph)
personal_graph1 <- read.graph("graphs/personal_insample_graph_1.graphml", "graphml")
layout <- mebn.plot_personal_effects(personal_graph1, 10, graph_layout)
```

**Subject 86**

```{r personal_graphvisu2, eval=TRUE, fig.height = 4, fig.width = 5, fig.align = "left", echo=FALSE, message=FALSE, warning=FALSE}
source("mebn/MEBN.r")
require(igraph)
personal_graph2 <- read.graph("graphs/personal_insample_graph_86.graphml", "graphml")
layout <- mebn.plot_personal_effects(personal_graph2, 10, graph_layout)
```
