...
Combine summary as a data frame for tables and sensitivity analysis
...
effects_table.df <- effects_table.df %>% 
  inner_join(ordered_typical_effects_rhs, by="effect") %>%
...
effects_table.df$var_rhs <- ordered_typical_effects_rhs$variance
...
kable(effects_table.df, booktabs = T, row.names = FALSE, 
  col.names = c("Nutrient or feature", "Concentration", "Inter-subject variation ($\\hat{\\sigma_b}$)","Var RHS", "Min. personal effect ($\\hat{\\boldsymbol{\\beta}} + \\hat{\\mathbf{b}}$)", "Max. personal effect ($\\hat{\\boldsymbol{\\beta}} + \\hat{\\mathbf{b}}$)","Min. RHS", "Max RHS")) %>%
  kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) 
