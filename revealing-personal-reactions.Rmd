...
mse.mat <- readRDS(file="evaluations/MSE-matrix2.rds")
mse.df <- data.frame(mse.mat[,1])

# model is a string factors, MSEs are doubles
mse.df$model <- factor(mse.mat[,1], levels = c("Normal","Gamma","Gamma AR(1)","Gamma AR(1) RHS","EXPFAM AR(1)","EXPFAM AR(1) RHS"))
mse.df <- cbind(mse.df, apply(mse.mat[,2:7], 2, as.numeric))[,2:8]

label.df <- as.data.frame(colnames(mse.mat)[2:7])
label.df <- cbind(label.df, as.numeric(mse.mat[6,2:7]))
...
# TODO: what about 12 weeks change?
class.df <- as.data.frame(as.vector(class_sum.mat[,5]))
class.df$model <- "MEBN"
...
mebn_cv.mat <- readRDS("evaluations/Class-matrix1.rds")
mebn_cv.df <- as.data.frame(mebn_cv.mat[,5]-2)
