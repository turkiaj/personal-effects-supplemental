...
author:
- JARI TURKIA
- School of Computing, University of Eastern Finland, 80101 Joensuu, Finland, and 
- CGI Suomi, Joensuu, Finland
- jari.turkia@cgi.com
- .
- VILLE HAUTAMÄKI, LAURI MEHTÄTALO
...
abstract: This notebook is supplementary material for the article "Revealing Personal
  Effects of Nutrition with Mixed-Effect Bayesian Network". The notebook shows in
...
```{r spagettiplot, echo=FALSE, fig.align="left", fig.height=7, fig.width=6, message=FALSE, fig.cap="Progress of concentration levels during the 12-week study. Measurements were taken at weeks 0, 4, 8, and 12, and the food records were kept during the week before the measurement"}
...
```{r graph, fig.height = 5, fig.align = "left", fig.cap="Initial, bi-partite, network structure.", echo=FALSE, message=FALSE}
library(igraph)
...
```{r normal_model_ppc, fig.cap="Visual posterior predictive check of local distributions with normal distribution.", echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}
...
```{r gamma_ppc, fig.cap="Visual posterior predictive check of local distributions with Gamma distribution.", echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, cache=FALSE}
...
```{r gamma_ar1_ppc, fig.cap="Visual posterior predictive check of local distributions with Gamma distribution and first-order autocorrelation process.", echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}
...
```
```{r ar1_comparison_table, fig.cap="Comparison of autocorrelation coefficients for each distribution.", echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.height=5}
...
```{r gamma_ar1_rhs_ppc, fig.cap="Visual posterior predictive check of local distributions with Gamma AR(1) distribution and regularized horseshoe prior on common effect coefficients.", echo=FALSE, fig.height = 4, fig.width=5, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}
...
```{r model_nrmse1, fig.cap="Comparison of NRMSE errors between model versions and their local distributions. ", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r fsins_comparison, fig.width=6, fig.cap="Plot on the left is a PPC of Gamma distributed fsins-response and the right plot is a normally distributed response that fits better with lower mean squared error", echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}
library(gridExtra)
...
```{r expfam_ar1_rhs_ppc, echo=FALSE, fig.height = 5, fig.width=5,fig.align = "left", message=FALSE, warning=FALSE, eval=FALSE, cache=FALSE}
source("mebn/MEBN.r")
...
kable(nrmse.mat, "latex", booktabs = T) %>%
...
```{r nrmse_plot_vertical, fig.cap="Comparison of NRMSE metrics of different model versions. Normaized error is measured each local concentration distribution and for the whole network as mean of local distribution errors.", fig.height = 3, fig.width = 5, echo=FALSE, fig.align = "left", message=FALSE, warning=FALSE, cache=TRUE}
library(ggplot2)
...
prediction_accuracy_plot <- ggplot() +
  geom_col(data = class.df, aes(x=Concentration, y=Accuracy, fill=Method), position = "dodge") +
...
summary_table.df$"fS-HDL" <- paste0(class.df[class.df$Concentration == "fS-HDL",]$Accuracy,"%")
summary_table.df$"fS-LDL" <- paste0(class.df[class.df$Concentration == "fS-LDL",]$Accuracy,"%")
summary_table.df$"fS-Ins" <- paste0(class.df[class.df$Concentration == "fS-Ins",]$Accuracy,"%")
summary_table.df$"fP-Gluc" <- paste0(class.df[class.df$Concentration == "fP-Gluc",]$Accuracy,"%")
summary_table.df$"fS-Chol" <- paste0(class.df[class.df$Concentration == "fS-Chol",]$Accuracy,"%")

kable(summary_table.df, booktabs = T) %>%
  kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) 

kable(summary_table.df, "latex", booktabs = T) %>%
  kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) %>%
...
```{r, typical_effects_figure, fig.height = 4, fig.cap="This visualization shows mean of posterior for typical coefficients as weight of the connection between blood concentrations and nutrients at diet affecting it. For clarity, the visualization only shows 15 principal components explaining the variance of each blood concentrations.", fig.width=5, echo=FALSE, fig.align = "left", message=FALSE, warning=FALSE, cache=FALSE}
source("mebn/MEBN.r")
require(igraph)
...
```{r personal_variations_figure, fig.height = 5, fig.align = "left", cache = FALSE, fig.cap="Graph visualization of inter-subject variation in nutritional effects. The effect of energy intake has one of the largest inter-subject variances at insulin response."}
source("mebn/MEBN.r")
...
personal.df <- data.frame(effect = character(0), value = numeric(0), value_lCI = numeric(0), value_uCI = numeric(0), row.names = NULL)
...
for (i in c(1:nrow(assumedtargets)))
{
  ...
  for (p in c(1:nrow(assumedpredictors)))
  {
    ...
    for (s in 1:patients)
    {
      ...
      personal.df <- rbind(personal.df, c(effectname,personal_quants[2],personal_quants[1],personal_quants[3]))
    }
  }
...
# Seek minimum and maximum of personal effects

colnames(personal.df) <- c("effect","personal_value","value_lCI","value_uCI")
personal.df$personal_value <- as.numeric(personal.df$personal_value)
personal.df$value_lCI <- as.numeric(personal.df$value_lCI)
personal.df$value_uCI <- as.numeric(personal.df$value_uCI)

personal_max_effects.df <- personal.df %>% 
  group_by(effect) %>%
  arrange(desc(personal_value)) %>%
  filter(row_number()==1) %>%
  rename(personal_max = personal_value, max_lCI = value_lCI, max_uCI = value_uCI) %>%
  as.data.frame()

personal_min_effects.df <- personal.df %>% 
  group_by(effect) %>%
  arrange(personal_value) %>%
  filter(row_number()==1) %>%
  rename(personal_min = personal_value, min_lCI = value_lCI, min_uCI = value_uCI) %>%
  as.data.frame()

```

Summary of findings as a table

```{r personal_effects_table}

effects_table.df <- data.frame(effect <- ordered_typical_effects$effect, row.names = NULL)
effects_table.df$effect <- factor(effects_table.df$effect, levels = effects_table.df$effect)

# join beta_rhs
effects_table.df <- effects_table.df %>% 
  inner_join(ordered_typical_effects_rhs) %>%
  transmute(effect = effect, beta_rhs = paste0(round(strength,2)," [",round(effect_lCI,2),"; ",round(effect_uCI,2),"]"))

effects_table.df$nutrient <- sapply(strsplit(ordered_typical_effects$effect, " -> "), function(x) x[1])
effects_table.df$conc <- sapply(strsplit(ordered_typical_effects$effect, " -> "), function(x) x[2])
effects_table.df$beta <- paste0(round(ordered_typical_effects$strength,2)," [",round(ordered_typical_effects$effect_lCI,2),"; ",round(ordered_typical_effects$effect_uCI,2),"]")
effects_table.df$variance <- paste0(round(ordered_typical_effects$variance,2)," [",round(ordered_typical_effects$variance_lCI,2),"; ",round(ordered_typical_effects$variance_uCI,2),"]")

# join the personal min-max to table
effects_table.df <- effects_table.df %>%
  inner_join(personal_min_effects.df) %>%
  inner_join(personal_max_effects.df) 

effects_table.df$min_beta_b <- paste0(round(effects_table.df$personal_min,2)," [",round(effects_table.df$min_lCI,2),"; ",round(effects_table.df$min_uCI,2),"]")
effects_table.df$max_beta_b <- paste0(round(effects_table.df$personal_max,2)," [",round(effects_table.df$max_lCI,2),"; ",round(effects_table.df$max_uCI,2),"]")

# order by inter-subject variance
effects_table.df$var <- ordered_typical_effects$variance
effects_table.df <- effects_table.df[order(effects_table.df$variance, decreasing = TRUE),]
effects_table.df$effect <- factor(effects_table.df$effect, levels = effects_table.df$effect)

# remove effect and var columns
effects_table.df <- effects_table.df[,c("nutrient","conc","variance","min_beta_b","max_beta_b")]
rownames(effects_table.df) <- NULL

kable(effects_table.df, booktabs = T, row.names = FALSE, 
  col.names = c("Nutrient or feature", "Concentration", "Inter-subject variation ($\\hat{\\sigma_b}$)", "Min. personal effect ($\\hat{\\boldsymbol{\\beta}} + \\hat{\\mathbf{b}}$)", "Max. personal effect ($\\hat{\\boldsymbol{\\beta}} + \\hat{\\mathbf{b}}$)")) %>%
  kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) 

kable(effects_table.df, "latex", booktabs = T, row.names = FALSE, escape = F, align="llrrrrrr",
      col.names = c("Nutrient or feature", "Concentration", "Inter-subject variation, $\\hat{\\sigma_b}$", "Min.", "Max.")) %>%
  add_header_above(c(" " = 5, "Personal effect, $\\hat{\\boldsymbol{\\beta}} + \\hat{\\mathbf{b}}$ and 90\\% CI" = 2), escape = F) %>%
  kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) %>%
...
```{r, echo=FALSE, eval=TRUE, fig.height=3, fig.cap="The elbow plot of k-means clustering shows that difference between clusters decreases when more than four clusters are considered. These clusters include all the predictors, nutrients and the cholesterol lowering medication."}
library(stats)
library(gridExtra)
...
```{r variation clusters1, echo=FALSE, eval=TRUE, fig.width=4, fig.height=5, cache=FALSE}
source("mebn/MEBN.r")
...
```{r personal_effect_clusters_all, echo=FALSE, eval=TRUE, fig.width=4, fig.height=5, cache=FALSE}
source("mebn/MEBN.r")
...
```{r, echo=FALSE, eval=TRUE, cache=FALSE, fig.height=3, fig.cap="Four clusters separated also when only the nutrient predictors of concentrations were considered."}
...
```{r variation clusters3, echo=FALSE, eval=TRUE, fig.width=4, fig.height=5, cache=FALSE}
source("mebn/MEBN.r")
# Number of cluster centers 
k <- 4
...
```{r personal_effect_clusters_nutrients, echo=FALSE, eval=TRUE, fig.width=4, fig.height=5, cache=FALSE}
...
```{r personal_graphs_comparison, eval=TRUE, fig.height = 8, fig.width = 5, fig.align = "left", echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
source("mebn/MEBN.r")
require(igraph)
...
colnames(pe)<-c("Subject", "Effect", "Lower-CI", "Mean value", "Upper-CI")
