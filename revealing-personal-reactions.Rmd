...
#Definition of the nutritional effects model
...
normal_vs_gamma <- mebn.LOO_comparison(assumedtargets, "BLMM_normal", "BLMM_gamma")
```
...
This coefficient 'ar1' is estimated separately for every response.
...
ar0_vs_ar1 <- mebn.LOO_comparison(assumedtargets, "BLMM_gamma", "BLMM_gamma/ar1")
...
Let us compare the model candidates with MSE
...
Execution of the k-fold cross-validation is done in a separated notebook (cross-validation.Rmd) as it takes substantial amount of time to run. The results are stored and analyzed here.

```{r, message=FALSE, warning=FALSE}
source("model_eval_functions.r")

# Collect the precitive performance of different models to same table
summary.mat <- matrix(0, nrow=0,ncol=6)
colnames(summary.mat) <- c('model', 'week 4', 'week 8', 'week 12', '12 week change', 'average accuracy')

if (!file.exists("evaluations/RHS-insample-eval.rds"))
{
  eval.mat <- mebn_rep_accuracy("RHS", "BLMM_gamma/ar1_rhs/no_holdout/", length(levels(sysdimet$SUBJECT_ID)), assumedtargets)
  saveRDS(eval.mat, "evaluations/RHS-insample-eval.rds")
} else {
  eval.mat <- readRDS("evaluations/RHS-insample-eval.rds")
}

summary.mat <- rbind(summary.mat, eval.mat)

if (!file.exists("evaluations/RHS-insample-eval.rds"))
{
  eval.mat <- mebn_rep_accuracy("no-RHS" ,"BLMM_gamma/ar1/", length(levels(sysdimet$SUBJECT_ID)), assumedtargets)
  saveRDS(eval.mat, "evaluations/no-RHS-insample-eval.rds")
} else {
  eval.mat <- readRDS("evaluations/no-RHS-insample-eval.rds")
}
