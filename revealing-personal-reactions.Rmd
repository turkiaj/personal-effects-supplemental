...
nrmse.df$model <- factor(nrmse.mat[,1], levels = rev(c("Normal","Gamma","Gamma AR(1)","Gamma AR(1) RHS","EXPFAM AR(1)","EXPFAM AR(1) RHS", "Gamma AR(1) RHS CV")))
...
ggplot(nrmse.df, show.legend = FALSE) +
  ...
  geom_text(data = label.df, aes(label = label, x = "Gamma AR(1) RHS CV", y = value, hjust = .4, vjust = (label == "fpgluk")*1.1+1.2), size = 3) +
...
mebn_rhs.df <- as.data.frame(mebn_rhs.mat[,5]-2)
...
mebn_cv.mat <- readRDS("evaluations/MEBNCV-matrix.rds")
mebn_cv.df <- as.data.frame(mebn_cv.mat[,5]-2)
mebn_cv.df$method <- "MEBN RHS CV"
...
title("Patient 1 from cluster 3",cex.main=0.5)
layout <- mebn.plot_personal_effects(personal_graph2, 10, graph_layout)
title("Patient 2 from cluster 2",cex.main=0.5)
```
