...
hdlplot <- ggplot() +
  ...
  scale_x_discrete(limits=c(0,3,4,7,8,11,12)) +
  geom_vline(xintercept =c(4,8,12)) + 
  annotate("rect",xmin=c(3,7,11), xmax=c(4,8,12), ymin=-Inf, ymax=Inf, alpha = .2) + 
...
p1 <- ggplot(ordered_typical_effects, aes(x=effect_text, y=strength)) + 
  ...
  labs(y=expression(paste(hat(beta),", vaque prior"))) +
  coord_flip()
